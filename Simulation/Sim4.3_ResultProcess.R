...
sig.density.ann <- c('Lower'='1% Signal')
...
for (Dist in c('Normal')) {
	...
	for (Mea in c('FDR', 'Power')) {
		...
		pdf(paste0("Figure14_", Dist, "_", Mea, "_grid_bar(AdaPT+)+.pdf"), width=8, height=2.5)
...
for (Dist in c('Normal')) {
	...
	for (Mea in c('FDR', 'Power')) {
		...
		pdf(paste0("Figure9_", Dist, "_", Mea, "_grid_bar(AdaPT)+.pdf"), width=8, height=2.5)
		obj <- ggplot(res3, aes(x=OrderPrior, y=Value,  fill=Method)) +
				geom_bar(stat='identity', position=dodge) +
				geom_errorbar(aes(ymax=ymax, ymin=ymin), position=dodge, width=0.25, size = 0.25)
