			...
			for (sig.dist in sig.dists) {
				...
				for (sig.density in sig.densities) {
					...
					for (sig.strength in sig.strengths) {
				        ...
				        data <- SimulateData9.4(prior.strength, feature.no, sig.dist, sig.density, sig.strength)
						prior <- data$prior
						pvalue <- data$pvalue
						truth <- data$truth
						pi0.true <- mean(!truth)
...
resdir <- paras$resdir
setwd(resdir)
res <- clsapply(1:100, func, paras, queque="1-day",  tempdir="~/project/orderfdr/mforge/Sim9.4")
setwd(resdir)
