...
func <- function(part, paras) {
	...
	res <- array(NA, c(nIter, length(prior.strengths), length(feature.nos), length(sig.dists), length(sig.densities), length(sig.strengths),
					length(methods), length(measures)), 
			dimnames=list(1:nIter, prior.strengths, feature.nos, sig.dists, sig.densities, sig.strengths, methods, measures))	
	for (iter in 1:nIter) {
				...
				for (sig.dist in sig.dists) {
					...
					for (sig.density in sig.densities) {
						...
						for (sig.strength in sig.strengths) {
							...
							data <- SimulateData4(prior.strength, feature.no, sig.dist, sig.density, sig.strength)
							prior <- data$prior
							pvalue <- data$pvalue
							truth <- data$truth
							pi0.true <- mean(!truth)
							...
							for (method in methods) {
								...
								if (is.null(res.obj[[method]])) {
									res[iter, prior.strength, paste(feature.no), sig.dist, sig.density, sig.strength, method, c('FDR', 'Power')] <- NA
									next
								} 
								...
								if (sum(fdr <= fdr.cutoff) == 0) {
									res[iter, prior.strength, paste(feature.no), sig.dist, sig.density, sig.strength, method, 'FDR'] <- 0
									res[iter, prior.strength, paste(feature.no), sig.dist, sig.density, sig.strength, method, 'Power'] <- 0
								} else {
									
									res[iter, prior.strength, paste(feature.no), sig.dist, sig.density, sig.strength, method, 'FDR'] <- mean(truth[fdr <= fdr.cutoff] == 0)
									res[iter, prior.strength, paste(feature.no), sig.dist, sig.density, sig.strength, method, 'Power'] <- sum(truth[fdr <= fdr.cutoff] != 0) / sum(truth != 0)
								}
							}
...
paras$prior.strengths <- c('Weak')
paras$sig.densities <- c('None')
paras$sig.strengths <- c('Weak')

resdir <- paras$resdir
setwd(resdir)
res <- clsapply(1:100, func, paras, queque="1-day",  tempdir="~/project/orderfdr/mforge/Sim4.4.new")
setwd(resdir)
