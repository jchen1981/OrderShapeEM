...
nIter <- 20
sig.dists <- c('Normal')
prior.strengths <- c('Weak')
sig.densities <- c('None')
sig.strengths <- c('Weak')
...
res.a <- array(NA, c(nIter, length(prior.strengths), length(feature.nos), length(sig.dists), length(sig.densities), length(sig.strengths),
				length(methods), length(measures), length(res)), 
		dimnames=list(InnerIter = paste(1:nIter), OrderPrior=prior.strengths, FeatureNumber=feature.nos, SignalDist=sig.dists,
...
for (Dist in c('Normal')) {
	...
	for (Mea in c('FDR')) {
		...
		pdf(paste0("Figure10_", Dist, "_", Mea, "_grid_bar+.pdf"), width=5.5, height=4)
		obj <- ggplot(res3, aes(x=Method, y=Value,  fill=Method)) +
